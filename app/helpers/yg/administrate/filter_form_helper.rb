...
module Yg
  module Administrate
    module FilterFormHelper
      def yg_administrate_filter_current_values(raw_values)
        values =
      ...
      def yg_administrate_filter_hour_options
        @yg_administrate_filter_hour_options ||= [["", ""]] + (0..23).map { |hour| [format("%02d", hour), format("%02d", hour)] }
      end

      def yg_administrate_filter_minute_options
        @yg_administrate_filter_minute_options ||= [["", ""], ["00", "00"], ["30", "30"], ["59", "59"]]
      end

      def yg_administrate_datetime_filter_parts(value, end_of_day: false)
      ...
      def yg_administrate_datetime_filter_combined_value(parts)
        if parts[:date].blank?
          ""
      ...
      def yg_administrate_checkbox_group_options(options, label_method: nil, value_method: nil)
